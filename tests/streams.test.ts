...
describe('multi-stream: view size hints', () => {
  it('forwards a watcher’s display size to the streamer, validated', async () => {
    ...
    bob.c.send({ type: 'view-size', streamer: alice.id, height: 360, fps: null })
    ...
    bob.c.send({ type: 'view-size', streamer: alice.id, height: 1e9, fps: null })
    bob.c.send({ type: 'view-size', streamer: alice.id, height: 720, fps: 0 })
